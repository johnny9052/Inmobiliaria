...
--
-- Servidor: 127.0.0.1
-- Tiempo de generación: 23-11-2018 a las 22:41:41
...
CREATE  PROCEDURE `listcontractpropertie` (`idcontrato` INT)  BEGIN
  select con.idcontrato as id, CONCAT(cli.primerNombreCliente, ' ', 
			...
			inm.matriculahexagono_inmobiliaria as matricula,
...
CREATE  PROCEDURE `listpropertiecsv` (`idfilter` INT)  BEGIN
	...
	SELECT distinct                 
               inm.matriculahexagono_inmobiliaria as matricula_hexagono_inmobiliaria,
               inm.precio as precio,
...
CREATE  PROCEDURE `loadinmueblestopvisitas` (IN `idfilter` INT)  BEGIN
	...
	IF idfilter > -1 THEN
		...
		select inm.matriculahexagono_inmobiliaria, count(*) as totalvisitas
                from inmuebles as inm 
                inner join  audit_propertie_search as aud 
                on aud.	id_register = inm.idinmueble
                group by inm.matriculahexagono_inmobiliaria
...
CREATE  PROCEDURE `loadpropertie` (IN `idfilter` INT)  BEGIN
	...
	IF idfilter > -1 THEN
		...
		select 	idinmueble as id,matriculahexagono_inmobiliaria  as nombre
		from inmuebles                
		ORDER BY nombre;
...
IF NOT EXISTS(select 1 from inmuebles where matriculahexagono_inmobiliaria=vMatriculahexagono_inmobiliaria and idinmueble<>vid)
		THEN
			update inmuebles set                                                                      
                                    ...
                                    matriculahexagono_inmobiliaria = vMatriculahexagono_inmobiliaria,
...
CREATE TABLE `inmuebles` (
  ...
  `matriculahexagono_inmobiliaria` varchar(45) DEFAULT NULL,
...
(4202, 'Administración hexagono_inmobiliaria', 1),
...
(85, 'hexagono_admin', 'Tiene todos los permisos, excepto configuracion de roles, usuarios y permisos de usuario');
...
ALTER TABLE `inmuebles`
  ...
  ADD UNIQUE KEY `matriculahexagono_inmobiliaria_UNIQUE` (`matriculahexagono_inmobiliaria`),
