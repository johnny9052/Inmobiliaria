...
--
-- Servidor: 127.0.0.1
-- Tiempo de generación: 30-10-2018 a las 21:29:23
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `listfeaturedpropertie` (`vfilter` INT)  BEGIN
   ...
   select DISTINCT  inm.idinmueble as id, tip_inm.`nombreTipoInmueble` as tipo,
               ...
               tip.`nombreTipoInmueble` as tipoinmueble
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `listpqrs` (`idfilter` INT)  BEGIN
   select pqr.idPQRS as id, tip.nombretipopqrs,pqr.fechaPQRS as fechaPQRS, pqr.descripcionPQRS as descripcionPQRS,
		pqr.respuestaDada as respuestaDada
   from pqrs as pqr inner join tipopqrs as tip 
   on tip.idtipopqrs = pqr.idtipopqrs
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `searchpqrs` (`vid` INT)  BEGIN
	...
	FROM pqrs as pqr inner join tipopqrs as tip 
   on pqr.idtipopqrs = tip.idtipopqrs
	where idPQRS = vid;	
...
CREATE DEFINER=`root`@`localhost` FUNCTION `savepqrsclient` (`vpqrstype` INT, `vdescription` VARCHAR(200), `vfecha` VARCHAR(20), `vhora` VARCHAR(20), `vname` VARCHAR(20), `vemail` VARCHAR(20)) RETURNS INT(1) READS SQL DATA
    DETERMINISTIC
...
(2, 0, '15:00:00', '', '', '', '', '', '', '', '1970-01-01', 0, 0, 0, 0, 0, 0, 0, NULL),
...
(4, NULL, 'David', 'Alberto', 'Angarita ', 'Garcia', NULL, NULL, '3002680564', 'davidangaritag@gmail.com', NULL, 0, 0, 0, 0, 0, 0, 0, 'e10adc3949ba59abbe56e057f20f883e');
...
INSERT INTO `pqrs` (`idPQRS`, `fechaPQRS`, `horaPQRS`, `descripcionPQRS`, `respuestaDada`, `idtipopqrs`) VALUES
(1, '2018-10-01', '10:00:00', 'Prueba petición', NULL, 1),
(2, '2018-10-12', '15:00:00', 'Prueba queja', 'Respuesta queja', 2);
